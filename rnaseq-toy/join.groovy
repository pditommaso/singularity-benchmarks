/* 
 * Read "plot_singularity.txt" and "plot_docker.txt" tab-separated files. 
...
def file1 = new File('plot_singularity.txt')
...
file1.readLines().each { line -> 
  ...
  all[index++] = [ name: name, singularity: time ]
}
...
println "name,realtime_docker,realtime_singularity"

all.each { record -> println "${record.name},${record.docker},${record.singularity}" }
