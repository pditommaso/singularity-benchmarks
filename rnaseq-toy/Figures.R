...
qplot(x=(realtime_docker/1000)/60, y=(realtime_singularity/1000)/60, data=NF_stats, main="", color=name, ylab="singularity tasks avg. time (mins)", xlab="Docker tasks avg. time (mins)") + 
    geom_point(size = 3, aes(shape = shape)) +
    geom_point( data = NF_median, aes(x=(realtime_docker/1000)/60, y=(realtime_singularity/1000)/60, shape = shape, color = NA), size = 3) +
    geom_point( data = NF_mean, aes(x=(realtime_docker/1000)/60, y=(realtime_singularity/1000)/60, shape = shape, color = NA), size = 3) +
